...
if(isset($_POST['readrecords'])){
	...
	$displayquery = " SELECT * FROM `conseil_c2s1` "; 
...
if(isset($_POST['numeroo']) && isset($_POST['matiere']) && isset($_POST['nom'])){
		...
		$query = " INSERT INTO `conseil_c2s1`(`numeroo`, `matiere`, `nom`) VALUES ( '$numeroo',  '$matiere', '$nom')";
...
if(isset($_POST['deleteidd']))
{
	...
	$deletequery = " delete from conseil_c2s1 where numeroo ='$user_id' ";
...
if(isset($_POST['numeroo']) && isset($_POST['numeroo']) != "")
{
    ...
    $query = "SELECT * FROM conseil_c2s1 WHERE numeroo = '$user_id'";
...
*/	$lastnamepd = $_POST['matiere'];
    $massarpd = $_POST['nom'];
				...
				$query = " UPDATE `conseil_c2s1` SET `matiere`='$lastnamepd',`nom`='$massarpd' WHERE numeroo= '$hidden_user_idupd' ";
				mysqli_query($conn,$query);
